'use strict';
/* ═══════════════════════════════════════════════════════════════
   SEED / MIGRATION – VBC Stainach CMS
   Migriert die ursprünglich statischen HTML-Inhalte in die
   JSON-Datei-Datenbank. Läuft beim Serverstart und befüllt nur
   Collections, die noch leer sind (idempotent – überschreibt nie
   vorhandene Daten).
═══════════════════════════════════════════════════════════════ */

module.exports = function seed(db) {

  /* ── Hilfsfunktion: nur befüllen, wenn Collection leer ────── */
  function seedIfEmpty(name, records) {
    if (db.all(name).length) return false;
    records.forEach(r => db.insert(name, r));
    console.log(`✔  Collection "${name}" mit ${records.length} Einträgen migriert`);
    return true;
  }

  /* ════════════════ STATISTIK-LEISTE (Startseite) ═══════════ */
  seedIfEmpty('stats', [
  ...
  /* ════════════════ TEAMS ═══════════════════════════════════ */
  const seededTeams = seedIfEmpty('teams', [
    {
      ...
      image: 'WebsiteAssets/Medien (2).jpg',
      email: 'vorstand@vbcstainach.com',
      sort_order: 1
    },
    {
      ...
      image: 'WebsiteAssets/Medien (5).jpg',
      email: '',
      sort_order: 2
    },
    {
      ...
      image: 'WebsiteAssets/Medien (4).jpg',
      email: 'herren2@vbcstainach.com',
      sort_order: 3
    }
  ]);

  /* ════════════════ SPIELER (Kader, Quelle: volleynet.at) ═══ */
  if (seededTeams || !db.all('players').length) {
    const teams = db.all('teams');
    const tid = key => (teams.find(t => t.key === key) || {}).id || 0;

    const h1 = [
      ['Manuel Aigner', 'Diagonal', 'Diagonalangreifer'],
      ['Matthias Buchberger', 'Libero', 'Libero'],
      ['Alexander Egger', 'Mitte', 'Mittelblocker'],
      ['Elias Gottsbacher', 'Spieler', '—'],
      ['Martin Hehn', 'Außen', 'Außenangreifer'],
      ['Florian Jäger', 'Mitte', 'Mittelblocker'],
      ['Roberto Klaric', 'Spieler', '—'],
      ['Stefan Krug', 'Außen', 'Außenangreifer'],
      ['Alexander Lichtenwöhrer', 'Zuspieler', 'Zuspieler'],
      ['Valentin Marschall', 'Spieler', '—'],
      ['Noah Meinzer', 'Mitte', 'Mittelblocker'],
      ['Nicklas Melbinger', 'Libero', 'Libero'],
      ['Johannes Neuwersch', 'Libero', 'Libero'],
      ['Matthias Neuwersch', 'Spieler', '—'],
      ['Wolfgang Neuwersch', 'Außen', 'Außenangreifer'],
      ['Alexander Oberegger', 'Zuspieler', 'Zuspieler'],
      ['Peter Prodinger', 'Spieler', '—'],
      ['Johannes Reisinger', 'Spieler', '—'],
      ['Maximilian Tschernitz', 'Spieler', '—'],
      ['Wilhelm Weiss', 'Außen', 'Außenangreifer']
    ];
    const d1 = [
      ['Stefanie Barth', 'Nr. 5'], ['Summer Berger', 'Nr. 8'],
      ['Sara Čelina', 'Nr. 11'], ['Andriana Drinovac', 'Nr. 4'],
      ['Isabel Gruber', 'Nr. 16'], ['Julia Gruber', 'Nr. 17'],
      ['Julia Kanzler', 'Nr. 20'], ['Chanel Kernberger', 'Nr. 9'],
      ['Lena Kühberger', 'Nr. 6'], ['Aruna Liebhart', 'Nr. 28'],
      ['Isabella Marković', 'Nr. 27'], ['Iva Martinovic', 'Nr. 10']
    ];
    const h2 = [
      ['Antonio Crncevic'], ['Michael Erlinger'], ['Xaver Oliver Fehér'],
      ['Julian Helm'], ['Max Peter Hochsteger'], ['Philipp Jaritz'],
      ['Martin Kral'], ['Alex Lichtenwöhrer', 'Zuspieler', 'Zuspieler'],
      ['Mateo Pocrnja'], ['Lukas Ruhdorfer'], ['Robert-Daniel Rusu'],
      ['Amar Salkanovic'], ['Raphael Schmid'], ['Gabriel-Christian Tulea'],
      ['Maximilian Tschernitz'], ['Rasmus Zaihsenberger'], ['Ilhan Zekan']
    ];
  ...
  seedIfEmpty('gallery', [
    ...
    { image: 'WebsiteAssets/Medien (5).jpg', alt: 'VBC Stainach Volleyball', span: 'normal', sort_order: 5 },
    { image: 'WebsiteAssets/Medien (6).jpg', alt: 'VBC Stainach Volleyball', span: 'normal', sort_order: 6 },
    { image: 'WebsiteAssets/Medien (7).jpg', alt: 'VBC Stainach Volleyball', span: 'normal', sort_order: 7 }
  ]);

  /* ════════════════ ZEITLEISTE (Geschichte) ═════════════════ */
  seedIfEmpty('timeline', [
    ...
    { year: '1985',    title: 'Vereinsgründung',          text: 'Offizielle Gründung des VBC Stainach/Irdning. Der Verein nimmt seinen Betrieb mit ersten Ligamannschaften auf.', sort_order: 2 },
    ...
    { year: 'Heute',   title: '3 aktive Mannschaften',    text: 'Mit 49 aktiven Spielern und drei Mannschaften schreibt der VBC Stainach/Irdning weiterhin seine Geschichte.', sort_order: 6 }
  ]);

  /* ════════════════ ERFOLGE ═════════════════════════════════ */
  seedIfEmpty('achievements', [
  ...
  /* ════════════════ SPIELE & SPONSOREN (initial leer) ═══════ */
  /* Werden über das Admin-Dashboard gepflegt – loadDb legt die
     Dateien bei Bedarf automatisch an. */
  db.all('games');
  db.all('sponsors');

  /* ════════════════ SEITENTEXTE (Key-Value) ═════════════════ */
  if (!db.all('pages').length) {
    ...
    const texts = {
      startseite: {
        ...
        sponsors_intro:   'Wir danken unseren Partnern für die wertvolle Unterstützung des VBC Stainach/Irdning.',
        about_title:      'Mehr als Volleyball —\neine Gemeinschaft',
        about_text1:      'Der VBC Stainach/Irdning ist der Volleyballverein der Region. Seit Jahrzehnten verbinden wir Menschen durch Sport, Teamgeist und gemeinsame Leidenschaft für das Spiel. Mit drei aktiven Mannschaften bieten wir Volleyball auf verschiedenen Leistungsniveaus — von ambitionierten Ligaspielerinnen bis zu aufstrebenden Jugendspielern.',
        ...
        history_text1:    'Die Geschichte des VBC Stainach/Irdning reicht weit zurück. Was einst als regionale Sportbegeisterung begann, ist heute ein etablierter Volleyballverein mit Tradition, Nachwuchs und überregionaler Bekanntheit.',
        ...
        history_text3:    'Highlights wie die Teilnahme von Florian Gosch an den Olympischen Spielen 2004 zeigen, welches Potenzial in diesem Verein steckt. Der VBC Stainach/Irdning ist mehr als ein Verein — er ist ein Stück Heimat.',
        gallery_intro:    'Momente aus dem Training, Spielen und dem Vereinsleben des VBC Stainach/Irdning.',
        ...
        footer_tagline:   'Volleyball, Teamgeist und Leidenschaft seit 1985 in Stainach/Irdning.'
      },
      teams: {
    ...
    for (const [page, sections] of Object.entries(texts)) {
      for (const [section, content] of Object.entries(sections)) {
        db.insert('pages', { page, section, content, updated_at: now });
        count++;
      }
    }
    console.log(`✔  Collection "pages" mit ${count} Textbausteinen migriert`);
  }
};
